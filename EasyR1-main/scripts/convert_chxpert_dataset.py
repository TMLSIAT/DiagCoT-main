...
def convert_medical_report_dataset(input_file, output_dir, split="train"):
    ...
    for item in tqdm(data, desc=f"Converting {split} data"):
        ...
        # 构建转换后的数据项
        converted_item = {
            "prompt": f"Based on this X-ray image, classify it according to the following fourteen labels (No Finding, Enlarged Cardiomediastinum, Cardiomegaly, Lung Opacity, Lung Lesion, Edema, Consolidation, Pneumonia, Atelectasis, Pneumothorax, Pleural Effusion, Pleural Other, Fracture, Support Devices), selecting the conditions you believe are present in the image. If there are no symptoms, select: No Finding. ", # <img>{image_path}</img>
            "images": [image_path],
            "answer": item["text"]
        }
    ...
    # 保存转换后的数据
    output_file = os.path.join(output_dir, f"chxpert_{split}.json")
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="/data0/zhuoxu/yihong/code/EasyR1-main/examples/data/medical_cls_chxpert", help="Output directory")
