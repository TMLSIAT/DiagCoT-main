...
"""
改进的医疗标框奖励函数
主要改进：在有框的情况下，IoU>=0.5时直接给1分，<0.5时直接给0分
这样可以提供更明确的奖励信号，避免模糊的中间奖励
"""
...
def improved_grounding_reward(predict: str, ground_truth: str) -> float:
    """
    改进的grounding任务奖励分数计算函数
    主要改进：在有框的情况下，IoU>=0.5时直接给1分，<0.5时直接给0分
...
def compute_score(predicts: List[str], ground_truths: List[str], format_weight: float = 0.2) -> List[Dict[str, float]]:
    ...
    for i, (predict, ground_truth) in enumerate(zip(predicts, ground_truths)):
        ...
        # 计算改进的IoU奖励
        iou_score = improved_grounding_reward(predict, ground_truth)
    ...
    if total_samples > 0:
        avg_format = np.mean(format_scores)
        avg_iou = np.mean(iou_scores)
        print(f"改进版最终统计: 平均格式奖励={avg_format:.3f}, 平均IoU奖励={avg_iou:.3f}")
        print(f"改进版IoU奖励分布: 0分: {sum(1 for s in iou_scores if s == 0.0)/len(iou_scores):.1%}, "
              f"1分: {sum(1 for s in iou_scores if s == 1.0)/len(iou_scores):.1%}")
...
def analyze_improved_iou_distribution(predicts: List[str], ground_truths: List[str]) -> Dict[str, Union[float, int]]:
    """
    分析改进版IoU分布情况，用于调试和监控
    Args:
        predicts: 预测结果列表
        ground_truths: 真实标签列表
    Returns:
    ...
    for predict, ground_truth in zip(predicts, ground_truths):
        ...
        if not true_boxes and not pred_boxes:
            ...
            # 计算改进的奖励
            reward = 1.0 if metrics['avg_iou'] >= 0.5 else 0.0
    ...
    if iou_values:
        stats = {
            ...
            'reward_1.0_rate': sum(1 for r in reward_values if r == 1.0) / len(reward_values),
            'reward_0.0_rate': sum(1 for r in reward_values if r == 0.0) / len(reward_values),
            'total_samples': len(predicts),
