...
# def content_reward(predict: str, ground_truth: str) -> Dict[str, float]:
#     """
#     评估生成报告的内容质量
...
#         candidate = word_tokenize(predict)
...
#         rouge_scores = scorer.score(ground_truth, predict)
...
#     # 关键医学发现匹配度评估
#     # 从参考报告中提取关键医学术语
#     medical_terms = extract_medical_terms(ground_truth)
#     if medical_terms:
#         # 计算生成报告中包含的关键医学术语比例
#         matched_terms = sum(1 for term in medical_terms if term in predict)
#         medical_term_score = matched_terms / len(medical_terms)
#     else:
#         medical_term_score = 0.0
...
#         "rouge2": rouge2_score,
#         "rougeL": rougeL_score,
#         "medical_term": medical_term_score
#     }
...
def compute_score(predicts: List[str], ground_truths: List[str], format_weight: float = 0.3) -> List[Dict[str, float]]:
    ...
    for predict, ground_truth in zip(predicts, ground_truths):
        ...
        #spine 要额外的提取impression中的内容
        ## 进一步提取Impression:后面的内容
        impression_match = re.search(r'Impression\s*:\s*([\s\S]*)', predicts_answer, re.IGNORECASE)
        
        if impression_match:
            # 如果找到Impression，只使用Impression后面的内容
            predicts_answer = impression_match.group(1).strip()
        # 如果没有找到Impression，继续使用完整的answer内容
        ...
        # 语义相似度
        # semantic_score = calculate_semantic_similarity(predicts_answer, ground_truth)
        
        # 长度适配
        # length_score = calculate_length_adjustment(predicts_answer, ground_truth)
        
        # 实体覆盖
        # entity_score = calculate_entity_coverage(predicts_answer, ground_truth)
