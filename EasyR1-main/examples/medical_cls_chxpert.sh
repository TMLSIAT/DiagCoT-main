...
DATA_DIR=${EASYR1_DIR}/examples/data/medical_cls_chxpert
...
MODEL_PATH=/data0/zhuoxu/yihong/code/EasyR1-main/Stage2_chxpert_lora_cls_model_w_system/merged_model
...
CUDA_VISIBLE_DEVICES=0,1 python3 -m verl.trainer.main \
    ...
    data.train_files=${DATA_DIR}/chxpert_train.parquet \
    data.val_files=${DATA_DIR}/chxpert_train.parquet \
    ...
    data.format_prompt=${EASYR1_DIR}/examples/format_prompt/medical_chxpert_cls_format.jinja \
    data.max_pixels=1000000 \
    data.min_pixels=10000 \
    worker.actor.model.model_path=${MODEL_PATH} \
    worker.actor.model.trust_remote_code=true \
    worker.reward.reward_function=examples/reward_function/medical_classification_reward_multichoice_2_improved.py:compute_multichoice_score_improved \
    ...
    trainer.experiment_name=medical_cls_chxpert_grpo \
    trainer.n_gpus_per_node=2 \
    trainer.total_epochs=1 \
    trainer.val_before_train=false \
    trainer.val_freq=0 \
    trainer.save_freq=200 \
    trainer.save_checkpoint_path=${EASYR1_DIR}/checkpoints_cls_new_3/easy_r1 \
    algorithm.adv_estimator=grpo
# trainer.save_freq=200 \ 
# worker.ref.fsdp.enable_cpu_offload=false \
# worker.actor.offload.offload_params=false \
# trainer.load_checkpoint_path=/data0/zhuoxu/yihong/code/EasyR1-main/checkpoints_cls_new/easy_r1/global_step_200 \
# # 训练完成后单独运行保存脚本
# CHECKPOINT_PATH=${EASYR1_DIR}/checkpoints_cls/easy_r1/medical_cls_grpo/final
# mkdir -p ${CHECKPOINT_PATH}

# # 复制保存的actor模型到最终目录
# cp -r ${EASYR1_DIR}/checkpoints/easy_r1/medical_report_vl_grpo/global_step_* ${CHECKPOINT_PATH}/
# echo "模型已保存至: ${CHECKPOINT_PATH}"
# data.max_pixels=1000000  远远不够！ 7000000往上才够 因为Chxpert这个数据的图像像素都很高