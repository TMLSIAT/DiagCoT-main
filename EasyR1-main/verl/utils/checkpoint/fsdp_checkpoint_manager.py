...
from torch.distributed.checkpoint.state_dict import StateDictOptions, get_state_dict, set_state_dict ,StateDictConfig,OptimStateDictConfig,StateDictType
...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    #原先的
        ...
        state_dict_options = StateDictOptions(cpu_offload=True) #True
        set_state_dict(
            model=self.model,
    ...
    #     # 使用内存映射加载大文件
    #     model_state_dict = torch.load(model_path, weights_only=False, map_location='cpu', mmap=True)
    #     optim_state_dict = torch.load(optim_path, weights_only=False, map_location='cpu', mmap=True)
