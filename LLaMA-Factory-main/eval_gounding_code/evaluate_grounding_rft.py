    ...
    try:
        ...
        with torch.no_grad():
            generated_ids = model.generate(
                **inputs,
                max_new_tokens=4096,
                use_cache=True,  # 保持RFT版本的use_cache参数
...
def main():
    ...
    parser = argparse.ArgumentParser(description="X-ray Grounding任务评估")
    parser.add_argument("--model_path", type=str, default="/data0/zhuoxu/yihong/code/EasyR1-main/checkpoints_grounding_merger_llm_train_new/easy_r1/global_step_200/actor/huggingface",
                       help="训练好的模型路径")
    parser.add_argument("--test_data", type=str, default="/data0/zhuoxu/yihong/code/EasyR1-main/eval_data/test_grounding_balanced.json",
                       help="测试数据文件路径")
    parser.add_argument("--output_dir", type=str, default="/data0/zhuoxu/yihong/code/LLaMA-Factory-main/task_grounding_result_balanced_improved_rft_step200_checkpoints_grounding_merger_llm_train_new",
                       help="评估结果输出目录")
    ...
    print("=" * 60)
    print("改进的X-ray Grounding任务评估 (RFT版本)")
    print("=" * 60)
    ...
    if "high_iou_samples" in metrics:
        ...
        if high_iou_stats['perfect_samples']['samples']:
            ...
            for i, sample in enumerate(high_iou_stats['perfect_samples']['samples'][:3]):
                ...
                print(f"    真实标注: {sample['ground_truth']['raw_annotation'][:100]}...")
                print(f"    模型预测: {sample['prediction']['raw_prediction'][:100]}...")
