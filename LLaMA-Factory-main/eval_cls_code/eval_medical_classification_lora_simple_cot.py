...
def evaluate_model(model_path: str, test_file: str, output_file: str = None, max_samples: int = None):
    ...
    for i, item in enumerate(tqdm(test_data, desc="评估进度")):
        ...
        # #提取answer标签内的内容
        # prediction = extract_answer_from_tags(prediction)

        # 提取标签 已经集成提取answer标签内的内容
...
def main():
    ...
    parser.add_argument('--model_path', type=str, default='/data0/zhuoxu/yihong/code/EasyR1-main/Stage2_chxpert_lora_cls_model_w_system_use_cot_5e-5_ep3_after_21w_lora/merged_model', help='模型路径')
    parser.add_argument('--test_file', type=str, default='eval_data/test_cls_qwen.json', help='测试文件路径')
    parser.add_argument('--output_file', type=str, default="/data0/zhuoxu/yihong/code/LLaMA-Factory-main/task_cls_result/lora_cls_qwen_simple_cot.json", help='输出结果文件路径')
